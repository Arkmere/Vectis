...
REFERENCE_SPECS = {
    ...
    "config/sensitive_regions.csv": ["COUNTRY", "ICAO_PREFIX", "REGION_NAME", "CATEGORY", "NOTES"],
...
def classify_callsign(arcid: str) -> dict[str, str]:
    ...
    if is_registration_callsign(code):
        return {"CALLSIGN_FORM": "REGISTRATION_CALLSIGN", "CALLSIGN_ROOT": code, "CALLSIGN_PREFIX3": code[:3]}
    ...
    if len(root) >= 4 and re.match(r"^[A-Z]{4,}\d", code):
        return {"CALLSIGN_FORM": "LONGFORM_NONSTANDARD", "CALLSIGN_ROOT": root, "CALLSIGN_PREFIX3": root[:3]}

    if re.match(r"^[A-Z]{3}[A-Z0-9]+$", code):
        return {"CALLSIGN_FORM": "TRICODE_STYLE", "CALLSIGN_ROOT": code[:3], "CALLSIGN_PREFIX3": code[:3]}

    return {"CALLSIGN_FORM": "UNKNOWN", "CALLSIGN_ROOT": root, "CALLSIGN_PREFIX3": root[:3] if root else ""}
...
def _read_reference(base_dir: Path, relative_path: str, headers: list[str]) -> pd.DataFrame:
    path = base_dir / relative_path
    try:
...
def triage_file(
    ...
    interest_codes = set(refs["config/interest_icao_codes.csv"]["CODE"].map(normalize_code)) - {""}
    interest_types = set(refs["config/interest_aircraft_types.csv"]["ATYP"].map(normalize_code)) - {""}
    sensitive_regs = set(refs["config/sensitive_registrations.csv"]["REG"].map(normalize_code)) - {""}
    longform_roots = set(refs["config/longform_roots.csv"]["ROOT"].map(normalize_code)) - {""}
    operator_codes = set(refs["vkb/operators.csv"]["CODE"].map(normalize_code)) - {""}

    locations_df = refs["vkb/locations.csv"].copy()
    locations_df["ICAO_NORM"] = locations_df["ICAO"].map(normalize_code)
    location_codes = set(locations_df["ICAO_NORM"]) - {""}
    military_location_codes = set(
        locations_df.loc[
            locations_df["TYPE"].map(lambda value: normalize_code(value) in {"MILITARY", "STATE", "DUAL_USE"}),
            "ICAO_NORM",
        ]
    ) - {""}
    ...
    region_prefix_to_name = {
        row["PREFIX_NORM"]: _clean(row["REGION_NAME"]) or _clean(row["COUNTRY"]) or row["PREFIX_NORM"]
    ...
    for idx, row in work.iterrows():
        ...
        if row["CALLSIGN_FORM"] == "LONGFORM_NONSTANDARD":
            _append_reason(reasons, idx, 1, f"LONGFORM_CALLSIGN:{root}", first_pass, first_reason, matched_field, matched_value, "ARCID", _clean(row["ARCID"]))
    ...
    for idx, row in work.iterrows():
        ...
        if row["CALLSIGN_FORM"] == "TRICODE_STYLE" and root in interest_codes:
            _append_reason(reasons, idx, 2, f"KNOWN_ICAO_INTEREST:{root}", first_pass, first_reason, matched_field, matched_value, "ARCID", _clean(row["ARCID"]))
            operational_interest[idx] = True
    ...
    for idx, row in work.iterrows():
        ...
        if atyp in interest_types:
            _append_reason(reasons, idx, 4, f"INTEREST_AIRCRAFT_TYPE:{atyp}", first_pass, first_reason, matched_field, matched_value, "ATYP", _clean(row["ATYP"]))
            operational_interest[idx] = True

    # Pass 5: Unknown operator tricodes.
    for idx, row in work.iterrows():
        root = normalize_code(row["CALLSIGN_ROOT"])
        if row["CALLSIGN_FORM"] == "TRICODE_STYLE" and root and root not in operator_codes and first_pass[idx] is None:
            _append_reason(reasons, idx, 5, f"UNKNOWN_OPERATOR_TRICODE:{root}", first_pass, first_reason, matched_field, matched_value, "ARCID", _clean(row["ARCID"]))
    ...
    for idx, row in work.iterrows():
        for field in ("ADEP", "ADES"):
            ...
            if code and code not in location_codes:
                _append_reason(reasons, idx, 6, f"{field}_NOT_IN_VKB:{code}", first_pass, first_reason, matched_field, matched_value, field, _clean(row[field]))
                vkb_update_required[idx] = True
    ...
    for idx, row in work.iterrows():
        for field in ("ADEP", "ADES"):
            ...
            if code in military_location_codes:
                _append_reason(reasons, idx, 7, f"{field}_VKB_MILITARY:{code}", first_pass, first_reason, matched_field, matched_value, field, _clean(row[field]))
                operational_interest[idx] = True
    ...
    for idx, row in work.iterrows():
        for field in ("ADEP", "ADES"):
            if normalize_code(row[field]) == "ZZZZ":
                _append_reason(reasons, idx, 8, f"{field}_ZZZZ", first_pass, first_reason, matched_field, matched_value, field, _clean(row[field]))
                operational_interest[idx] = True
    ...
    for idx, row in work.iterrows():
        for field in ("ADEP", "ADES"):
            ...
            if prefix:
                ...
                _append_reason(reasons, idx, 9, f"{field}_SENSITIVE_REGION:{region_name}", first_pass, first_reason, matched_field, matched_value, field, _clean(row[field]))
                operational_interest[idx] = True

    work["EXTRACTED_PASS"] = [PASS_NAMES.get(pass_no, "") if pass_no else "" for pass_no in first_pass]
    ...
    output_columns = original_columns + [column for column in ADDED_COLUMNS if column not in original_columns]
    ...
    missing_operator_counts = (
        work["ALL_MATCH_REASONS"].str.extractall(r"UNKNOWN_OPERATOR_TRICODE:([^;]+)")[0].value_counts().sort_index()
        if row_count else pd.Series(dtype=int)
    )
    missing_location_counts = (
        work["ALL_MATCH_REASONS"].str.extractall(r"(?:ADEP|ADES)_NOT_IN_VKB:([^;]+)")[0].value_counts().sort_index()
        if row_count else pd.Series(dtype=int)
    )
    update_rows = [
    ...
    update_candidates = pd.DataFrame(update_rows, columns=["CANDIDATE_TYPE", "CODE", "COUNT"])
    ...
    summary_rows = [
        ...
        {"METRIC": "missing operator tricodes", "VALUE": int(len(missing_operator_counts))},
        {"METRIC": "missing locations", "VALUE": int(len(missing_location_counts))},
        {"METRIC": "output timestamp", "VALUE": datetime.now().isoformat(timespec="seconds")},
    ]
    for pass_name, count in work.loc[work["EXTRACTED_FLAG"], "EXTRACTED_PASS"].value_counts().sort_index().items():
        summary_rows.append({"METRIC": f"count by first pass: {pass_name}", "VALUE": int(count)})
    summary = pd.DataFrame(summary_rows, columns=["METRIC", "VALUE"])

    sheets: dict[str, pd.DataFrame] = {
        "01_EXTRACTED_ALL": extracted,
        "02_LONGFORM_CALLSIGNS": extracted[extracted["EXTRACTED_PASS"] == PASS_NAMES[1]],
        "03_KNOWN_ICAO_INTEREST": extracted[extracted["EXTRACTED_PASS"] == PASS_NAMES[2]],
        "04_KNOWN_SENSITIVE_REGS": extracted[extracted["EXTRACTED_PASS"] == PASS_NAMES[3]],
        "05_INTEREST_AIRCRAFT_TYPES": extracted[extracted["EXTRACTED_PASS"] == PASS_NAMES[4]],
        "06_UNKNOWN_OPERATOR_TRICODES": extracted[extracted["EXTRACTED_PASS"] == PASS_NAMES[5]],
        "07_VKB_LOCATION_GAPS": work[work["ALL_MATCH_REASONS"].map(lambda value: _reason_contains(value, ("ADEP_NOT_IN_VKB", "ADES_NOT_IN_VKB")))],
        "08_VKB_MILITARY_LOCATIONS": work[work["ALL_MATCH_REASONS"].map(lambda value: _reason_contains(value, ("ADEP_VKB_MILITARY", "ADES_VKB_MILITARY")))],
        "09_ZZZZ_AERODROMES": work[work["ALL_MATCH_REASONS"].map(lambda value: _reason_contains(value, ("ADEP_ZZZZ", "ADES_ZZZZ")))],
        "10_SENSITIVE_REGIONS": work[work["ALL_MATCH_REASONS"].map(lambda value: _reason_contains(value, ("ADEP_SENSITIVE_REGION", "ADES_SENSITIVE_REGION")))],
